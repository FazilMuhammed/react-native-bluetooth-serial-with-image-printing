                  ...
                  rejector:(RCTPromiseRejectBlock)reject)
{
    ...
    if (message != nil) {
        ...
        [self printImage:image alignment:HLTextAlignmentLeft maxWidth:280];
        resolve((id)kCFBooleanTrue);
    } else {
        NSError *err = nil;
        reject(@"no_data", @"Data was null", err);
...
- (void) printImage:(UIImage *)image alignment:(HLTextAlignment)alignment maxWidth:(CGFloat)maxWidth {
    // 1.设置对齐方式
    NSLog(@"1.设置对齐方式");
    ...
    [_bleShield write:restoreTextSpace];
